...
# --- Configuration ---
CSV_FILE_PATH = 'support_tickets/support_ticket_description.csv'
TICKET_SUBJECT_COLUMN = 'case_subject'
TICKET_DESCRIPTION_COLUMN = 'description'
CASE_ID_COLUMN = 'case_id'
PROMPT_FILENAME = "support_ticket_nodes.txt"
...
TICKET_ASSIGNMENTS_FILENAME = os.path.join(OUTPUT_DIR, "support_ticket_nodes.json")
# Final output file with merged counts
FINAL_GRAPH_FILENAME = os.path.join(OUTPUT_DIR, "consolidated_graph_with_counts.json")
...
def process_tickets_in_chunks(df, model_name, prompt_template, unique_nodes):
    ...
    for i in range(num_chunks):
        ...
        ticket_chunk = list(zip(
            ...
            df[TICKET_SUBJECT_COLUMN][start_idx:end_idx],
...
# --- NEW FUNCTION ---
def aggregate_and_merge_ticket_data(ticket_assignments_file, consolidated_graph_file, output_file):
    """
    Aggregates ticket assignments from chunks and merges them into the main knowledge graph.
    """
    print("\n--- Aggregating and Merging Support Ticket Data ---")
    ...
    for node in nodes_in_graph:
        ...
        ticket_list = sorted(list(set(aggregated_tickets.get(entity_name, []))))
        ...
        node["properties"]["support_ticket_count"] = ticket_count
        node["properties"]["support_tickets"] = ticket_list
        if ticket_count > 0:
            updated_nodes_count += 1

    print(f"Merged ticket data into the consolidated graph. {updated_nodes_count} nodes were updated.")
    print(f"A total of {total_tickets_in_graph} unique support ticket references were added to the graph nodes.")
...
def main():
    parser = argparse.ArgumentParser(description="Extract and process support ticket nodes.")
    args = parser.parse_args()

    print("--- Starting Support Ticket Node Extraction ---")
    ...
    required_columns = [CASE_ID_COLUMN, TICKET_SUBJECT_COLUMN, TICKET_DESCRIPTION_COLUMN]
    ...
    for i in range(1, 5):
      kg_json_with_nodes = f"consolidated_graph_pass{i}.json"
      final_graph_filename = os.path.join(OUTPUT_DIR, f"consolidated_graph_with_counts_pass{i}.json")
      ticket_assignments_filename = os.path.join(OUTPUT_DIR, f"support_ticket_nodes_pass{i}.json")
      
      consolidated_graph_path = os.path.join(OUTPUT_DIR, kg_json_with_nodes)
      unique_nodes = load_unique_nodes_from_json(consolidated_graph_path)
      if not unique_nodes:
          print(f"Fatal: Could not load unique nodes from JSON: {consolidated_graph_path}")
          continue
      print(f"Loaded {len(unique_nodes)} unique nodes from consolidated graph: {consolidated_graph_path}")
      ...
      aggregate_and_merge_ticket_data(ticket_assignments_filename, consolidated_graph_path, final_graph_filename)

    

    print("\n--- Support Ticket Node Processing Complete ---")
    print(f"Final output saved to: {FINAL_GRAPH_FILENAME}")
